const { cartService, productService } = require("../repositories/service");

class TicketController {
    
    ticketPost = async (req, res) => {
        const { cid } = req.params;
        const cart = await cartService.getCarts(cid);
        ...
        const stock = await productService.getProductById(product._id);
        ...
        if (quantity > stock) {
            ...
            await productService.updateProduct(product._id, stock - quantity);
        }
        }
