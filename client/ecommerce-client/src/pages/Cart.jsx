...
const CartPage = () => {
        ...
        const calculateCartDetails = async () => {
            ...
            const fetchProductDetails = async () => {
                const productPromises = cart.map(async (item) => {
                    const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/products/${item.product}`, {
                        headers: {
    ...
    const handleRemoveProduct = async (productId) => {
        try {
            const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/carts/${user.cart}/product/${productId}`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${token}`,
